...
function problemB() {
  ...
  /* promise version:
El test asi no me corre, pero apartando el test, deberia de ser lo mismo si descomento linea
82 y en linea 83 aplico promise.all() ???
 var promise = new Promise();
 
 IGUALMENTE, ASI EL TEST CORRIA PERO NO CUMPLE CON EL ENUNCIADO:*/
  // Promise.all([
...
function problemC() {
  ...
  // -----------------------------------------------------------------------
  //DE ESTA MANERA FUNCIONA PERO NO ES LO MAS CORRECTO:
  // promisifiedReadFile("poem-one/stanza-02.txt").then((stanza2)=>{
  //   blue(stanza2);
  //   promisifiedReadFile('poem-one/stanza-03.txt').then((stanza3)=>
  //   blue(stanza3))
  ...
  //--------------------------------------------------------------------------
  //USARE .all() recordar que el all se "imprime" en el orden que le doy inicialmente:
  // Promise.all([
  //   promisifiedReadFile("poem-one/stanza-02.txt"),
  //   promisifiedReadFile("poem-one/stanza-03.txt"),
  // ]).then((stanzas) =>
  ...
  // );
  //-------------------------------------------------------------------------------
  //OTRA MANERA: Encadenamiento, me gusto mas!!:
  promisifiedReadFile("poem-one/stanza-02.txt")
    .then((stanza2) => {
      blue(stanza2);
...
function problemE() {
  ...
  // ])
  //   .then((stanzas) => stanzas.forEach((s) => blue(s)))
...
function problemF() {
  ...
  // ???
  //----------------------------------------------------------
  //QUE TENGO MAL ACA???: creo q promise.all no retorna nada si consigue
  //un error antes
  // Promise.all([
  //   promisifiedReadFile("poem-one/stanza-03.txt"),
  //   promisifiedReadFile("poem-one/wrong-file-name.txt"),
  ...
  promisifiedReadFile("poem-one/stanza-03.txt")
    ...
    .then((stanza4) => {
      blue(stanza4);
      //console.log("done"); //si sale bien, aca esta el console.log() esperando pero,
      //y si no sale bien?.. pues, agrego lo mismo al catch. INtente con finally
      //pero aun no veia porque no me corria haciendolo.
    })
    .catch((err) => {
      magenta(new Error(err));
      //console.log("done");
