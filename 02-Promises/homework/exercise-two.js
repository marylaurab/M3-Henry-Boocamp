...
function problemB() {
  ...
  // promise version
  // ???
  Promise.all(filenames.map((f) => promisifiedReadFile(f))).then((stanzas) => {
    stanzas.forEach((s) => blue(s));
    console.log("done");
  });
}
...
function problemC() {
  ...
  // promise version
  // ???
let promises=filenames.map(f=>promisifiedReadFile(f))
promises.reduce((prevProm,currentProm)=>{
  prevProm.then((stanza)=>{
    blue(stanza);
    return currentProm
  })
