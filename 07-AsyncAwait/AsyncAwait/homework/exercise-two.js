...
function problemA() {
  ...
  // AsyncAwait version
  async function stanza(file) {
    blue(await promisifiedReadFile(file));
  }
  async function all() {
    await Promise.all([
      stanza("poem-two/stanza-01.txt"),
      stanza("poem-two/stanza-02.txt"),
    ]);
    console.log("done");
  }
  all();
...
function problemB() {
  ...
  // );
  // AsyncAwait version
  async function readPoem2() {
    let arrayPromises = await Promise.all(
      filenames.map((f) => promisifiedReadFile(f))
    );
    arrayPromises.forEach((p) => blue(p));
    console.log("done");
  }
  readPoem2();
}
...
function problemD() {
  ...
  // AsyncAwait version ahora lo hago con un for:
